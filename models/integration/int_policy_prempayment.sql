...
CASE
WHEN {{ this }}.CLAIM_SURROGATE_KEY IS NULL
...
LEFT JOIN
{{ this }} ON {{ this }}.CLAIM_SURROGATE_KEY = md5(CONCAT(p.POLICYID, p.CUSTOMERID))
)
SELECT * FROM joined_data

{% if is_incremental() %}
WHERE CLAIM_SURROGATE_KEY NOT IN (SELECT CLAIM_SURROGATE_KEY FROM {{ this }})
OR CLAIM_SURROGATE_KEY IN (SELECT CLAIM_SURROGATE_KEY FROM {{ this }} WHERE updated_at < CURRENT_TIMESTAMP())
{% endif %}